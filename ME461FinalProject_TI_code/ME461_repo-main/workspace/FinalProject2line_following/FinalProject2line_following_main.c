...
//
// TITLE:  Final Project
// Here we implemented something similar to the previous file, except now we will be doing two line following. Due to the nature of how close the camera is
// to the floor, the two lines will be at the center of the robot, rather than the robot be at the center of the two lines.
//#############################################################################
...
// Count variables are intialized and pre-defined to being 0
...
//final project variables that are defined here. They are named extern since we are using them mainly in the chip and blob detection code. 
extern float cx1;
extern float cx2;
...
__interrupt void cpu_timer1_isr(void) {
    ...
    cent_avg = (cx1+cx2)/2;
    centerr = cent_avg -80;// The x-coordinate at the center of the screen is 80.
    // Calculate the robot's deviation from the center of the line by subtracting the average of the current x-coordinates of the 2 largest blobs by the center coordinate.
    ...
    case 10:
        ...
        turn = -0.013*(centerr); // // Scale the error and feed into the turn expression. This allows fine tunining of the turning mechanism in order to  bring the robot back on track.
...
//    case 20:
        ...
        // this is the same code used in one-line following. 
        if (timeint < 750) {   
            casenum = 10;
            crash_counter = 0;
            crash = 0;
        ...
        // move back to make a room to rotate
        ...
        // stop for 1s before resuming tracking
